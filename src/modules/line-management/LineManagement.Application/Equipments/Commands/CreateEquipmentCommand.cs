...
public sealed record CreateEquipmentCommand(CreateEquipmentDTO Dto) : IRequest<Result<Guid>>;
    ...
    : IRequestHandler<CreateEquipmentCommand, Result<Guid>>
{
    ...
    {
        var productionLine = await productionLineRepository.GetByIdAsync(request.Dto.ProductionLineId, cancellationToken);
        ...
        var equipment = Equipment.Create(request.Dto.Name, request.Dto.Description, request.Dto.ProductionLineId, dateTimeProvider.UtcNow);
